 ...
 */
static void hexone(key *restrict keys, gencode *restrict final)
{
 ...
 */
static void hextwo(const key *restrict keys, gencode *restrict final)
{
    ...
    /* two instructions */
    for (i=0; i<u32_nbits; ++i)
    {
        if ((a&((uint32_t)1<<i)) != (b&((uint32_t)1<<i))) break;
    }
    /* h2b: 4,6 */
    sprintf(final->line[0], "  uint32_t rsl = ((val << %" PRIu32 ") & 1);\n", i);
}
 ...
 */
static void hexthree(key *restrict keys, gencode *restrict final, const hashform *restrict form)
{
    uint32_t a = keys->hash_k;
    uint32_t b = keys->next_k->hash_k;
    ...
    if (x != y && x != z && y != z)
    {
        if (form->perfect == NORMAL_HP || (x != 3 && y != 3 && z != 3)) 
        {
            ...
            sprintf(final->line[0], "  uint32_t rsl = (val >> %" PRIu32 ");\n", (uint32_t)(u32_nbits-2));
        }
        else
        {
            /* h3d: 7fffffff, bfffffff, ffffffff */
            sprintf(final->line[0], "  uint32_t rsl = ((val >> %" PRIu32 ") ^ %" PRIu32 ");\n",
                    (uint32_t)(u32_nbits-2), find_adder(x,y,z));
        }
        return;
    }
    ...
    for (i=0; i<final->highbit; ++i)
    {
        ...
        if (x != y && x != z && y != z)
        {
            if (form->perfect == NORMAL_HP || (x != 3 && y != 3 && z != 3))
            {
                ...
                sprintf(final->line[0], "  uint32_t rsl = ((val >> %" PRIu32 ") & 3);\n", i);
            }
            else
            {
                /* h3f: ffff7fff, ffffbfff, ffffffff */
                sprintf(final->line[0], "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ %" PRIu32 ");\n", i,
                        find_adder(x,y,z));
            }
            return;
        }
    ...
    for (i=0; i<=final->highbit; ++i)
    {
        ...
        if (x != y && x != z && y != z)
        {
            if (form->perfect == NORMAL_HP || (x != 3 && y != 3 && z != 3))
            {
                ...
                sprintf(final->line[0], "  uint32_t rsl = ((val+(val>>%" PRIu32 "))&3);\n", i);
            }
            else
            {
                /* h3h: 0x001, 0x100, 0x101 */
                sprintf(final->line[0], "  uint32_t rsl = (((val+(val>>%" PRIu32 "))&3)^%" PRIu32 ");\n", i,
                        find_adder(x,y,z));
            }
            return;
        }
            ...
            if (x != y && x != z && y != z)
            {
                if (form->perfect == NORMAL_HP || (x != 3 && y != 3 && z != 3))
                {
                    ...
                    sprintf(final->line[0], 
                            "  uint32_t rsl = (((val>>%" PRIu32 ") ^ (val>>%" PRIu32 ")) & 3);\n", i, j);
                }
                else
                {
                    /* h3j: 0x04, 0x10, 0x14 */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((((val>>%" PRIu32 ") ^ (val>>%" PRIu32 ")) & 3) ^ %" PRIu32 ");\n",
 ...
 */
static void hexfour(key *restrict keys, gencode *restrict final)
{
    uint32_t a = keys->hash_k;
    uint32_t b = keys->next_k->hash_k;
    uint32_t c = keys->next_k->next_k->hash_k;
    ...
    if (((final->diffbits >> (u32_nbits-2)) & 3) == 3)
    {
        w = a>>(u32_nbits-2);
        x = b>>(u32_nbits-2);
        y = c>>(u32_nbits-2);
    ...
    if ((final->diffbits & 3) != 0)
    {
        for (i=final->lowbit; i<=final->highbit; ++i)
        {
                ...
                ((final->diffbits & 2) != 0))
            {
                ...
                {                                                    /* h4h: 1,2,6,8 */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((val & 3) ^ ((val >> %" PRIu32 ") & 1));\n", i);
                    return;
                }
                ...
                {                                                    /* h4i: 1,2,8,a */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((val & 2) ^ ((val >> %" PRIu32 ") & 1));\n", i);
                    return;
                }
            }
                ...
                ((final->diffbits & 1) != 0))
            {
                ...
                {                                                    /* h4j: 0,1,3,4 */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((val & 3) ^ ((val >> %" PRIu32 ") & 2));\n", i);
                    return;
                }
                ...
                {                                                    /* h4k: 1,4,7,8 */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((val & 1) ^ ((val >> %" PRIu32 ") & 2));\n", i);
                    return;
                }
            }
        }
        ...
        if (((final->diffbits >> i) & 1) != 0)
        {
            for (j=final->lowbit; j<=final->highbit; ++j)
            {
                if (((final->diffbits >> j) & 3) != 0)
                {
                    ...
                    {                                                  /* h4o: 0,4,8,a */
                        sprintf(final->line[0], 
                                "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ ((val >> %" PRIu32 ") & 1));\n", 
                                j, i);
                        return;
                    }
                    ...
                    {                                   /* h4p: 0x04, 0x08, 0x10, 0x14 */
                        sprintf(final->line[0], 
                                "  uint32_t rsl = (((val >> %" PRIu32 ") & 2) ^ ((val >> %" PRIu32 ") & 1));\n", 
                                j, i);
                        return;
                    }
                }
                ...
                if (testfour(w,x,y,z))
                {
                    ...
                    {
                        sprintf(final->line[0], 
                                "  uint32_t rsl = (((val >> %" PRIu32 ") ^ (val << 1)) & 3);\n",
                                j);
                    }
                    else if (i==1)                         /* h4r: 0x01,0x09,0x0b,0x10 */
                    {
                        sprintf(final->line[0], 
                                "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ (val & 2));\n",
                                j);
                    }
                    else                                               /* h4s: 0,2,6,8 */
                    {
                        sprintf(final->line[0], 
                                "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ ((val >> %" PRIu32 ") & 2));\n",
                                j, (i-1));
                    }
                    return;
                }
                ...
                {                   
                    sprintf(final->line[0], 
                            "  uint32_t rsl = (((val >> %" PRIu32 ") & 1) ^ ((val >> %" PRIu32 ") & 2));\n",
                            j, i);
                    return;
                }
            }
     ...
     */
    for (i=final->lowbit; i<u32_nbits; ++i)
    {
        y = (c>>i)&1;
        z = (d>>i)&1;
        if (y != z)
    ...
    for (j=final->lowbit; j<u32_nbits; ++j)
    {
        x = ((b>>i)&1)^(((b>>j)&1)<<1);
        y = ((c>>i)&1)^(((c>>j)&1)<<1);
        z = ((d>>i)&1)^(((d>>j)&1)<<1);
    ...
    for (k=final->lowbit; k<u32_nbits; ++k)
    {
    ...
    if (i==u32_nbits || j==u32_nbits || k==u32_nbits)
    {
    ...
    /* now try the four cases */
    {
        ...
        if (testfour(w,x,y,z))
        {
            ...
            {
                sprintf(final->line[0], 
                        "  uint32_t rsl = (((val^(val>>%" PRIu32 "))&1)^((val>>%" PRIu32 ")&2));\n", o, n-1);
            }
            else                                            /* 0x00,0x04,0x10,0x12 */
            {
                sprintf(final->line[0], 
                        "  uint32_t rsl = ((((val>>%" PRIu32 ") ^ (val>>%" PRIu32 ")) & 1) ^ ((val>>%" PRIu32 ") & 2));\n",
                        m, o, n-1);
            }
            return;
        }
        ...
        if (testfour(w,x,y,z))
        {
            ...
            {
                sprintf(final->line[0], 
                        "  uint32_t rsl = ((val & 1) ^ (((val>>%" PRIu32 ") ^ (val>>%" PRIu32 ")) & 2));\n",
                        n-1, o-1);
            }
            else if (o==0)                                  /* 0x00,0x04,0x05,0x10 */
            {
                sprintf(final->line[0], 
                        "  uint32_t rsl = (((val>>%" PRIu32 ") & 2) ^ (((val>>%" PRIu32 ") ^ val) & 1));\n",
                        m-1, n);
            }
            else                                            /* 0x00,0x02,0x0a,0x10 */
            {
                sprintf(final->line[0], 
                        "  uint32_t rsl = (((val>>%" PRIu32 ") & 1) ^ (((val>>%" PRIu32 ") ^ (val>>%" PRIu32 ")) & 2));\n",
                        m, n-1, o-1);
            }
            return;
        }
        ...
        {
            final->used = 2;
            sprintf(final->line[0], "  uint32_t b = (val >> %" PRIu32 ") & 1;\n", o);
            if (m==o-1 && m==0)                             /* 0x02,0x10,0x11,0x18 */
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = ((val & 3) ^ ((val >> %" PRIu32 ") & 2) ^ b);\n", n-1);
            }
            else if (m==o-1)                                            /* 0,4,6,c */
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ ((val >> %" PRIu32 ") & 2) ^ b);\n",
                        m, n-1);
            }
            ...
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = (((val >> %" PRIu32 ") & 3) ^ b ^ (b << 1));\n", m);
            }
            else if (o==n-1 && m==0)                          /* h4am: not reached */
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = ((val & 1) ^ ((val >> %" PRIu32 ") & 3) ^ (b <<1 ));\n",
                        o);
            }
            else if (o==n-1)                                /* 0x00,0x02,0x08,0x10 */
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = (((val >> %" PRIu32 ") & 1) ^ ((val >> %" PRIu32 ") & 3) ^ (b << 1));\n",
                        m, o);
            }
            else if ((m != o-1) && (m != n-1) && (o != m-1) && (o != n-1))
            {
                ...
                {
                    sprintf(final->line[1], "  uint32_t b = -(newval >> %" PRIu32 ");\n", o);
                }
                if (m==0)                                     /* 0x00,0x04,0x09,0x10 */
                {
                    sprintf(final->line[2], 
                            "  uint32_t rsl = ((newval ^ (newval>>%" PRIu32 ") ^ b) & 3);\n", n-1);
                }
                else                                          /* 0x00,0x03,0x04,0x10 */
                {
                    sprintf(final->line[2], 
                            "  uint32_t rsl = (((newval>>%" PRIu32 ") ^ (newval>>%" PRIu32 ") ^ b) & 3);\n",
                            m, n-1);
                }
            }
            else if (o == m-1)
            {
                ...
                {
                    sprintf(final->line[0], "  uint32_t b = (val>>%" PRIu32 ") & 2;\n", o-1);
                }
                
                if (o==0)                                     /* 0x02,0x03,0x0a,0x10 */
                {
                    sprintf(final->line[1],
                            "  uint32_t rsl = ((val & 3) ^ ((val>>%" PRIu32 ") & 1) ^ b);\n",
                            n);
                }
                else                                          /* 0x00,0x02,0x04,0x10 */
                {
                    sprintf(final->line[1],
                            "  uint32_t rsl = (((val>>%" PRIu32 ") & 3) ^ ((val>>%" PRIu32 ") & 1) ^ b);\n",
                            o, n);
                }
            }
            ...
            {
                sprintf(final->line[1], 
                        "  uint32_t rsl = (((val>>%" PRIu32 ") & 1) ^ ((val>>%" PRIu32 ") & 2) ^ b ^ (b<<1));\n",
                        m, n-1);
            }
...
static bool testeight(key *restrict keys, uint8_t badmask)
{
 ...
 */
static bool hexeight(key *restrict keys, uint32_t nkeys, gencode *final, const hashform *restrict form)
{
    ...
    for (i=final->lowbit; i<=final->highbit-2; ++i)
    {
        ...
        {                                                                 /* h8b */
            final->used = 1;
            sprintf(final->line[0], "  uint32_t rsl = ((val >> %" PRIu32 ") & 7);\n", i);
            return true;
        }
    }
    ...
    for (i=final->lowbit; i<=final->highbit; ++i)
    {
        for (j=i+1; j<=final->highbit; ++j)
        {
            ...
            if (testeight(keys, badmask))
            {
                ...
                if (i == 0)                                                   /* h8c */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = ((val + (val >> %" PRIu32 ")) & 7);\n", j);
                else                                                          /* h8d */
                    sprintf(final->line[0], 
                            "  uint32_t rsl = (((val >> %" PRIu32 ") + (val >> %" PRIu32 ")) & 7);\n", i, j);
                return true;
            }
        ...
        for (j=i+1; j<=final->highbit; ++j)
        {
            for (k=j+1; k<=final->highbit; ++k)
            {
                ...
                {                                                             /* h8i */
                    final->used = 1;
                    sprintf(final->line[0], 
                            "  uint32_t rsl = (((val >> %" PRIu32 ") + (val >> %" PRIu32 ") + (val >> %" PRIu32 ")) & 7);\n", 
                            i, j, k);
                    return true;
                }
            }
 ...
 */
static void hexn(key *restrict keys, uint32_t salt, uint32_t alen, uint32_t blen, gencode *restrict final)
{
    key *mykey;
    ...
    for (;;)
    {
        ...
        case 1:
            ...
            for (mykey=keys; mykey; mykey=mykey->next_k)
            {
                mykey->a_k = (mykey->hash_k << (u32_nbits-(highbit+1)))>>(u32_nbits-alog);
        ...
        case 2:
            ...
            for (mykey=keys; mykey; mykey=mykey->next_k)
            {
                ...
                mykey->b_k = (mykey->hash_k << (u32_nbits-(highbit+1)))>>(u32_nbits-blog);
            }
            if (highbit+1 == u32_nbits)                                       /* hne */
                sprintf(final->line[5], "  b = (val >> %" PRIu32 ");\n",
                        u32_nbits-blog);
            else                                                            /* hnf */
                sprintf(final->line[5], "  b = ((val << %" PRIu32 " ) >> %" PRIu32 ");\n",
                        u32_nbits-(highbit+1), u32_nbits-blog);
            if (lowbit == 0)                                                /* hng */
        ...
        case 4:
            ...
            for (mykey=keys; mykey; mykey=mykey->next_k)
            {
                ...
                mykey->a_k = (mykey->hash_k << (u32_nbits-final->k-1)) >> (u32_nbits-alog);
            }
            if (final->j == 0)                                              /* hni */
                sprintf(final->line[5], "  b = val & 0x%" PRIx32 ";\n",
                        blen-1);
            else if (blog+final->j == u32_nbits)                             /* hnja */
                sprintf(final->line[5], "  b = val >> %" PRIu32 ";\n",
                        final->j);
            else
                sprintf(final->line[5], "  b = (val >> %" PRIu32 ") & 0x%" PRIx32 ";\n",      /* hnj */
                        final->j, blen-1);
            if (u32_nbits-final->k-1 == 0)                                    /* hnk */
                sprintf(final->line[6], "  a = (val >> %" PRIu32 ");\n",
                        u32_nbits-alog);
            else                                                            /* hnl */
                sprintf(final->line[6], "  a = ((val << %" PRIu32 ") >> %" PRIu32 ");\n",
                        u32_nbits-final->k-1, u32_nbits-alog);
        ...
        case 5:
            ...
            {
                if ((((final->diffbits<<(u32_nbits-final->k-1))>>alog) & (alen-1)) > 0)
                    break;
            }
            if (!(final->k < highbit))
            {
        ...
        case 6:
            /*
             * cases 6,7,8:
             * for (k=0; k<u32_nbits-alog; ++k)
             *   for (j=0; j<u32_nbits-blog; ++j)
             *     val = val+f(salt);
             *     val ^= (val >> 16);
             *     val += (val << 8);
             *     val ^= (val >> 4);
            ...
            /* Just do something that will surely work */
        {
            ...
            for (mykey=keys; mykey; mykey=mykey->next_k)
            {
                ...
                if (final->k == 0)
                    mykey->a_k = val >> (u32_nbits-alog);
                else
                    mykey->a_k = (val + (val << final->k)) >> (u32_nbits-alog);
            }
...
static void setlow(key *restrict keys, gencode *restrict final)
{
    uint32_t  i;
    key *mykey;
    uint32_t  firstkey;
 ...
 */
bool inithex(
    key      *restrict keys,   /* list of all keys */
    ...
    gencode  *restrict final,  /* output, code for the final hash */
    const hashform *form)   /* user directives */
{
    setlow(keys, final);
